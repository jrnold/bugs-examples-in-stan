...
```{r judges_setup,message=FALSE,cache=FALSE}
library("pscl")
library("tidyverse")
library("rstan")
```
...
Legislators example), estimating both the locations of the justices on a latent ideological dimension, and two parameters specific to each case (corresponding to the item difficulty and item discrimination parameters of a two-parameter IRT model).  The data consist of the decisions of Justices Rehnquist, Stevens, O'Connor, Scalia, Kennedy, Souter, Thomas, Ginsberg and Bryer, in that order, $i = 1, \dots , 9$. The decisions are coded 1 for votes with the majority, and 0 for votes against the majority, and `NA` for abstentions.
...
the legislator's ideal point ($\theta_i$), the vote's difficulty parameter and the vote's discrimination ($\beta_j$):
$$
\begin{aligned}[t]
